...
class DocumentoRepository {
	 ...
	 * @return Conjunto de documentos com os metadados.23456
	 */
	public Set<Documento> busca(Map<String, String> metadados) {
		Set<Documento> documentosBuscados = new HashSet<>();
		for (Documento doc: this.documentos.values()) {
			if (doc.getMetadados().keySet().containsAll(metadados.keySet())
					&& doc.getMetadados().values().containsAll(metadados.values())) {
				documentosBuscados.add(doc);
			}
		}
