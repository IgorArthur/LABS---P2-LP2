...
class BuscaRepository {
	...
	/**
	 * Cadastra uma BuscaSimples no histórico.
	 * 
	...
	/**
	 * Cadastra uma BuscaAvançada no histórico.
	 *
	 * @param buscaAvançada Operação de busca realizada.
	 * @param documentos   Documentos resultantes dessa busca.
	 */
	public void adicionaBusca(BuscaAvançada buscaAvançada, DocumentoDTO[] documentos) {
		String[] ids = Stream.of(documentos)
				.map(DocumentoDTO::getId)
				.collect(Collectors.toList())
				.toArray(new String[] {});
		this.buscas.add(new HistoricoBusca(buscaAvançada, ids));
	}
